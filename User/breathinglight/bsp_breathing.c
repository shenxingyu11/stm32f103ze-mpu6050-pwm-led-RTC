...
static void TIMx_Mode_Config(void)
{
	...
	#STM32系统时钟频率和周期
	f_pclk = 72000000
	t_pclk = 1/f_pclk 1/72000000

	#定时器update事件周期
	t_timer = t_pclk*TIMER_TIM_Prescaler*TIMER_TIM_Period  == 1/（72000000/200）*1024
  ...
  /* 基本定时器配置 */		  
  TIM_TimeBaseStructure.TIM_Period = (1024);;       							  //当定时器从0计数到 TIM_Period+1 ，为一个定时周期
...
//允许在定时器工作时向ARR的缓冲器中写入新值，以便在更新事件发生时载入覆盖以前的值。
  /* TIM3 enable counter */
